...
def test_transaction_descriptions(transaction):
    descriptions_transaction = transaction_descriptions(transaction)
    assert next(descriptions_transaction) == "Перевод организации"
    assert next(descriptions_transaction) == "Перевод со счета на счет"
    assert next(descriptions_transaction) == "Перевод со счета на счет"
    assert next(descriptions_transaction) == "Перевод с карты на карту"
    assert next(descriptions_transaction) == "Перевод организации"
